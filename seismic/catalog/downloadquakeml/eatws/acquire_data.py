...
def get_station_information(earthquake_id):
    url = GA_EATWS_API + "wfs?service=WFS&request=getfeature&typeNames=earthquakes:earthquakes_stations&outputFormat=application/json&CQL_FILTER=earthquake_id=" + str(earthquake_id)
    response = requests.get(url)
    return response.json()


def get_magnitudes_information(earthquake_id):
    url = GA_EATWS_API + "wfs?service=WFS&request=getfeature&typeNames=earthquakes:earthquakes_magnitude&outputFormat=application/json&CQL_FILTER=earthquake_id=" + str(earthquake_id)
    response = requests.get(url)
    return response.json()
...
def acquire_native_eatws_data(start_time, end_time):

    list_of_events = get_list_of_events(start_time, end_time)
