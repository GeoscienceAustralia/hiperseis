...
dirwrk = fullfile('/Users','nita','GEOPHYSICS', 'H_K', 'RF_HK');
netcode = 'AU';
stacode = 'KMBL';
vpdir = 'Vp_Tomo';
bazdir = 'BAZ_w0.360';
gausdir = 'Gauss_1.0';
psdir = 'RF_HK';
dirsacs = fullfile('/Users','nita','GEOPHYSICS', 'H_K', 'RF_HK');
ascf = ['sfr2d.', netcode, '.', stacode, '.x9'];
sumf = ['hkr2d.', netcode, '.', stacode,'.x9.00'];

nmfactor = 0.8;     % 0.5 or 0.8


%---- H-k summary file -----------------
file_sum = fullfile(dirwrk, vpdir, bazdir, sumf)
...
file_sfun = fullfile(dirwrk, vpdir, bazdir, ascf);
...
file_list = fullfile(dirwrk, [stacode, '_rayps.list']);
...
file_sht = fullfile(dirwrk, [stacode, '_shift.list']);
...
figure(11);
clf;
h111 = subplot('Position',[0.1, 0.1, 0.8, 0.8])  %[0.1, 0.1, 0.7, 0.7]);  %[left bottom width height]
...
xlim([moho_lb, 60]);
ylim([1.56, 1.9]) 
xlabel('Moho (km)', 'FontWeight','Bold', 'FontSize', 16);
ylabel('Vp/Vs', 'FontWeight','Bold', 'FontSize', 16);
set(h111, 'FontWeight','Bold', 'FontSize', 16);
...
%set(h112, 'FontWeight','Normal', 'FontSize', 16);
%xlim([0, 60]);
% xlim([moho_lb, 50]);
%ylim([-1.0, 1.0]);
...
%set(h113, 'FontWeight','Normal', 'FontSize', 16);
%xlim([-1.0, 1.0]);
...
ylabel('Ray Parameter (sec/deg)', 'FontWeight','Bold', 'FontSize', 16);
%set(ax,'YTickLabel','');
...
clf;
plot(rayp_hk, pmax', 'X');
xlabel('Ray Parameter', 'FontWeight','Bold', 'FontSize',16);
ylabel('Max RF amplitude', 'FontWeight','Bold', 'FontSize',16);
...
h1 = polar(0, 0);
set(h1, 'LineStyle','none', 'Marker','p', 'Color','r', 'MarkerFaceColor', 'y', 'MarkerSize', 16);
