	...
	while itr1s < lentr1_all and itr2s < lentr2_all:
		...
		while wtr1s < itr1e and wtr2s < itr2e:
			...
			if (sr1 < sr2):
				...
				rf = zeropad_ba(fftn(zeropad(tr1_d,fftlen1),shape=outdims1),fftlen2) * fftn(zeropad(ndflip(tr2_d),fftlen2),shape=outdims2)
			elif (sr1 > sr2):
				...
				outdims2 = np.array([fftlen2])
				outdims1 = np.array([fftlen1])
				rf = fftn(zeropad(tr1_d,fftlen1),shape=outdims1) * zeropad_ba(fftn(zeropad(ndflip(tr2_d),fftlen2),shape=outdims2),fftlen1)
			else:
...
def IntervalStackXCorr(refstn,st,
	...
	for tr_1 in refstn:
		...
		for tr_2 in st:
			...
			xcorr_func = xcorr2(tr_1, tr_2, window_seconds,interval_seconds)
...
def saveXCorrPlot(xcorr_list,xcorr_x_list,plot_output_dir,figname):
	...
	for i, comp_xcorr in enumerate(comp_list):
		...
		ax[0,i].pcolormesh( xcorr_x_list[i], np.arange(xcorr_list[i].shape[0]),xcorr_list[i], vmin=0, vmax=1, cmap='jet') # , label=comp_xcorr, lw=1.2)
