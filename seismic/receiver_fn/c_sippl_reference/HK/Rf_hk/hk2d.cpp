...
/*----------------------------------------------------------------------------*/
    ...
    hks->verbose = 0;                      // =1: Output xcorr sfunction for illustration
                                                        // Only works for X = 1  
...
/*----------------------------------------------------------------------------*/
    while ((c=getopt(argc, argv, "P:Z:K:W:T:N:X:R:C:L:I:D:O:osvh")) != -1) {
        switch (c) {
            case 'P':
...
/*----------------------------------------------------------------------------*/
/* Print and check -I: DIR of RF data list file                               */
/*----------------------------------------------------------------------------*/        
    if (rfdir == NULL) {
        rfdir = new char[8];
        strcpy(rfdir, "./");
    }   
    printf("RF data list dir = '%s'\n", rfdir);
...
/*----------------------------------------------------------------------------*/
            if (bazinfo->lb <= bazinfo->ub) { 
                if (baz >= bazinfo->lb && baz < bazinfo->ub) {
                    ...
                    strcpy(rf_file, rfdir); 
                    if (rfdir[strlen(rfdir)-1] != '/') strcat(rf_file, "/");        
...
/*----------------------------------------------------------------------------*/
/* if xc_flag == 10, do hks->xc_flag = 1, 2, 3, 7, 8, 9 one by one.
    Because hks->w_0p1s, hks->w_2p1s, hks->w_1p2s for the case hks->xc_flag = 0
    are usually different from those cases hks->xc_flag > 0, it has to be 
...
void help()
{
    ...
    printf("Option = -I, Argument = dir of RF data.\n");
    printf("Option = -L, Argument = RF data file list: The file name of list of time traces.\n");
