function head = rsacheader1(sacfilename, machineformat)
%
% Read one single sac file header, list all defined information
% Input:
% sacfilename: sac file name including dir
% machineformat: machine data format choice, consisten with fopen
% Output:
% return head as sac header struct
%
% Open binary file as IEEE floating point with big-endian byte ordering,
% since SAC file is written in Unix.
...
%--- Using the following to accurately determine delta, b and time serial ---
delta = 1 / round(1/headf(1, 1));
t_tmp = 0:delta:abs(headf(2, 1)) + delta;
tdif = t_tmp - abs(headf(2, 1));
[~, indx] = min(abs(tdif));
b = sign(headf(2, 1)) * t_tmp(indx);
npts = headi(2, 5);
e = delta * (npts - 1) + b;
...
%for ii = 3:4
%   for jj = 1:5
%      if headf(ii,jj) ~= -12345.0
%         disp(['   T',int2str(ij),'(',int2str(ii),',',int2str(jj),')', num2str(headf(ii,jj))]);
%         ij = ij + 1;
%      end
%   end
%end
%ij = 0;
%for ii = 3:5
%   if isempty(findstr(char(headc(ii,1:8)), '-12345'))
%      disp(['  KT',int2str(ij),'(',int2str(ii),',1) = ', char(headc(ii,1:8))]);
%   end
%   ij = ij + 1;      
%   if isempty(findstr(char(headc(ii,9:16)), '-12345'))
%      disp(['  KT',int2str(ij),'(',int2str(ii),',2) = ', char(headc(ii,9:16))]);
%   end
%   ij = ij + 1;      
%   if isempty(findstr(char(headc(ii,17:24)), '-12345'))
%      disp(['  KT',int2str(ij),'(',int2str(ii),',3) = ', char(headc(ii,17:24))]);
%   end
%   ij = ij + 1;
%end
%if isempty(findstr(char(headc(6,1:8)), '-12345'))
%   disp(['  KT9(9,1) = ', char(headc(6,1:8))]);
%end
