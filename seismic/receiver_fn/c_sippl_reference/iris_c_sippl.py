...
def get_ttimes(indict, outdir, stat_lat, stat_lon, mode='IRIS', files='day_mseed', phase='P', stat='KMBL', netwrk='AU',
               interval=[-60, 120]):
    """
    ...
    for j in indict.keys():
        ...
        int(eid), indict[j]['orig_time'].year, indict[j]['orig_time'].month, indict[j]['orig_time'].day,
        indict[j]['orig_time'].hour, indict[j]['orig_time'].minute,
        round(indict[j]['orig_time'].second + (indict[j]['orig_time'].microsecond / 10e6), 3), indict[j]['event_lat'],
        indict[j]['event_lon'], indict[j]['event_depth'], phase, ttime, tof)
        # create directory, download waveform data
        dirname = outdir + '/' + str(eid) + '__' + str(indict[j]['orig_time'].year) + str(
            indict[j]['orig_time'].month) + str(indict[j]['orig_time'].day) + '_' + str(
            indict[j]['orig_time'].hour) + str(indict[j]['orig_time'].minute) + str(indict[j]['orig_time'].second)
        ...
        if mode == 'IRIS':
            ...
            if files == 'hr_mseed':
                tracename_1 = '%4s%02i%02i%02i%02i' % (
                stat, indict[j]['orig_time'].year - 2000, indict[j]['orig_time'].month, indict[j]['orig_time'].day,
                indict[j]['orig_time'].hour)
                print(tracename_1)
                if not indict[j]['orig_time'].hour == 23:
                    tracename_2 = '%4s%02i%02i%02i%02i' % (
                    stat, indict[j]['orig_time'].year - 2000, indict[j]['orig_time'].month, indict[j]['orig_time'].day,
                    indict[j]['orig_time'].hour + 1)
                else:
                    tracename_2 = '%4s%02i%02i%02i00' % (
                    stat, indict[j]['orig_time'].year - 2000, indict[j]['orig_time'].month,
                    indict[j]['orig_time'].day + 1)
                try:
                ...
                tracename_base = '%02i%02i%02i' % (
                int(str(indict[j]['orig_time'].year)[2:]), indict[j]['orig_time'].month, indict[j]['orig_time'].day)
                ...
                try:
                    etr = read(glob.glob(
                        mode + '/' + str(indict[j]['orig_time'].year) + '/' + netwrk + '/' + stat + '/HHE.D/*.' + jday)[
                                   0])
                    etr.merge(fill_value='interpolate')
                    print(len(etr))
                    etrace = etr[0]
                    ntr = read(glob.glob(
                        mode + '/' + str(indict[j]['orig_time'].year) + '/' + netwrk + '/' + stat + '/HHN.D/*.' + jday)[
                                   0])
                    ntr.merge(fill_value='interpolate')
                    ntrace = ntr[0]
                    ztr = read(glob.glob(
                        mode + '/' + str(indict[j]['orig_time'].year) + '/' + netwrk + '/' + stat + '/HHZ.D/*.' + jday)[
                                   0])
                    ztr.merge(fill_value='interpolate')
                    ztrace = ztr[0]
                except Exception:
                ...
                etrace.stats.starttime = UTCDateTime(indict[j]['orig_time'].year, indict[j]['orig_time'].month,
                                                     indict[j]['orig_time'].day, 0, 0, 0)
                ntrace.stats.starttime = UTCDateTime(indict[j]['orig_time'].year, indict[j]['orig_time'].month,
                                                     indict[j]['orig_time'].day, 0, 0, 0)
                ztrace.stats.starttime = UTCDateTime(indict[j]['orig_time'].year, indict[j]['orig_time'].month,
                                                     indict[j]['orig_time'].day, 0, 0, 0)
