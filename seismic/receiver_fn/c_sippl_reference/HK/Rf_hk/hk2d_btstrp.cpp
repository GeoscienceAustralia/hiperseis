...
int wmh_rv2(char* fpth, HKS_INFO *hks, RECORD *rfrec, BAZ_INFO *bazinfo, BOOTSTRAP &btstrp);
...
/*----------------------------------------------------------------------------*/
	...
	btstrp.flag = 0;                       // -B: n  Default not bootstrap  
	btstrp.n = 1;
...
/*----------------------------------------------------------------------------*/
	while ((c=getopt(argc, argv, "P:Z:K:W:T:N:X:R:C:L:H:D:O:B:osh")) != -1) {
		switch (c) {
			case 'P':
...
/* Print -B: bootstrap                                                        */
/*----------------------------------------------------------------------------*/
	if (btstrp.flag) printf("Set boostrap, use %d RF traces\n", btstrp.n);
	else printf("Unset boostrap\n");	
...
/* Calculate travel time: ts, tp
	They are NOT the travel times of 0p1s, 2p1s, and 1p2s starting from P
	Note how the mh and rv sort into 1-D array                                 */
/*----------------------------------------------------------------------------*/
		printf("Calculate travel time, ts and tp for all rfs...\n");
...
/*----------------------------------------------------------------------------*/
		if (statis(hks, btstrp.flag)) return -1;
...
/*----------------------------------------------------------------------------*/
/* Generate hks->nrfrec radom numbers in [0, hks->nrfrec-1] 
	If irec-th radom number == k, increase rfrec[k].resmpl_flag by 1           */	
/*----------------------------------------------------------------------------*/
		rand_low = 0;
		rand_high = hks->nrfrec-1;
		time(&seconds);
		srand((unsigned int) seconds);

		for (irec = 0; irec < hks->nrfrec; irec++) {
			rfrec[irec].TpTs(hks);   //Note how the mh and rv sort into 1-D array
		}
		
		btstrp.frv = new float[btstrp.n];
		btstrp.fmh = new float[btstrp.n];
		for (ibt = 0; ibt < btstrp.n; ibt++) {
			printf("\nBootstrap: %d\n", ibt);
			
			for (irec = 0; irec < hks->nrfrec; irec++) {
				rfrec[irec].n_resmpl = 0;
			}
			for (irec = 0; irec < hks->nrfrec; irec++) {
				rfrec[rand() % (rand_high - rand_low + 1) + rand_low].n_resmpl++;
			}
...
/*----------------------------------------------------------------------------*/
			...
			btstrp.frv[ibt] = hks->msf[0].frv;	
			btstrp.fmh[ibt] = hks->msf[0].fmh;
...
/* Output H-k search results into hkr2d.xxx.00 and/or .01 .02                 */	
/*----------------------------------------------------------------------------*/
		if (create_hkfname(fpth_hk, fodir1, fhk, hks->xc_flag, 1)) return -3;

		if (wmh_rv2(fpth_hk, hks, rfrec, bazinfo, btstrp)) {
 			fprintf(stderr, "\nError in writing mh_rv in '%s'!!\n\n", fpth_hk);
 			return -1;
		}	
...
{
	printf("Usage:\n");
	printf("moho2d -P 6.5254 -Z 10.0/100.0/0.1 -K 1.50/2.00/0.01 -W 0.6/0.2/0.2 -T 0.1 -N n \n"); 
	printf("       -L list -H 3 -s\n");
	printf("       -D dirout -O sfunc/mh_rv2d \n");
	...
	printf("Option = -X, Argument = XC weight. Default = 1. 0: No XC weight; \n"); 
	...
	printf("Option = -C, Argument = Apeak/mh/rv. The values control peak search. \n");
	...
	printf("Option = -B, Argument = Number of bootstraps.\n");
