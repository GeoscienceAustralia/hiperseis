...
def _xcorrelate(f, g, leadin=None):
    """
...
def s_iter_deconv_pulsetrain(numerator, denominator, sampling_rate, time_shift, max_pulses=1000,
    ...
    max_causal_idx = int(npts - time_shift*sampling_rate - 1)
    assert max_causal_idx > 0

    # correlate the signals
    xc = _xcorrelate(f_hat, g_hat, max_causal_idx)

    if only_positive:
        shift_index = np.argmax(xc)
    else:
        xc_abs = np.abs(xc)
        shift_index = np.argmax(xc_abs)
    # end if
    ...
    while (np.abs(d_error) > tol) and (num_pulses < max_pulses):
        ...
        xc = _xcorrelate(resid, g_hat, max_causal_idx)
...
def p_iter_deconv_pulsetrain(numerator, denominator, sampling_rate, time_shift, max_pulses=1000,
                             tol=1.0e-3, gwidth=2.5, only_positive=False, log=None):
    """
    Iterative deconvolution of source and response signal to generate seismic receiver function.
    Adapted to Python by Andrew Medlin, Geoscience Australia (2019), from Chuck Ammon's
...
def rf_iter_deconv(response_data, source_data, sr, tshift, both_sided=False,
    ...
    :type time_shift: float
    :param both_sided: P-RFs invoke one-sided (causal) deconvolution, whereas S-RFs
                       require deconvolution of both causal and acausal parts
    :type both_sided: bool
    ...
    for numerator in response_data:
        ...
        if(both_sided):
            rf_trace, _, _, _, fit = s_iter_deconv_pulsetrain(numerator, denominator, sampling_rate, tshift, **kwargs)
        else:
            rf_trace, _, _, _, fit = p_iter_deconv_pulsetrain(numerator, denominator, sampling_rate, tshift, **kwargs)
        # end if
