...
def create_chunk(out_dir, trace, st_time, end_time, sta):
    trace.trim(starttime=st_time, endtime=end_time)
    st_out = Stream(traces=[trace, ])
    dest_dir = os.path.join(out_dir, str(trace.stats.starttime.timetuple().tm_year), str(trace.stats.starttime.timetuple().tm_yday))
