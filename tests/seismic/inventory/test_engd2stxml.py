...
def test_engd2stxml_main(tmp_path):
    ...
    """
    assert not os.path.exists(os.path.join(tmp_path, "INVENTORY_" + rt_timestamp + ".xml"))
    ...
    engd2stxml.main(os.path.join(test_data_path, "IRIS-ALL_tiny.xml"),
                    stations_folder, tmp_path, test_mode=True)
    assert os.path.exists(os.path.join(tmp_path, "INVENTORY_" + rt_timestamp + ".xml"))
