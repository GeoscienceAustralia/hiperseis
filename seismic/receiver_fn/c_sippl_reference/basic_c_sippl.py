...
def read_info(info_path, header=False):
    ...
    for line in dat:
        sta, seis, sta3, seis_ser, dig, dig_ser, dum, lat, lon, elev, pregain, fsamp, name, net, resp_E, resp_N, resp_Z, path, pref = line.strip(
            '\n').split(None)
        if sta[
            0] == '_':  # station with seismometer change; go back to last line without _, get station name and archive from there; write different configurations
            sta_new = sta[1:]
            if not sta_new in infodict.keys():
                print(
                    'Warning: station name beginning with _ only allowed if indicating a configuration change to a peviously (!) defined station - station ignored!')
                continue
...
class Station:
    ...
    def response(self, channel='Z', time='None'):
        ...
        if len(self.changetimes) == 0:
            ...
            if time == 'None':
                print(
                    'Error: Seismometer type cannot be uniquely determined (i.e. a device change occurred at some point in time); please provide a timestamp (UTCDateTime object) with the keyword argument time!')
            else:
    ...
    def uptime(self, channel='pref', comp='Z'):
        ...
        for y in yrs:
            updict[y] = []
            llist = glob(
                self.archive + '/' + str(y) + '/' + self.network + '/' + self.name + '/' + channel + comp + '.D/*')
            for fl in llist:
    ...
    def scan(self, ident='HH'):
        ...
        """
        print(
            'This calls obspy-scan for station ' + self.name + '...this will take several minutes or up to an hour, depending on data volume. Results will be saved in file ' + self.name + '_scan.pdf!')
        os.system(
            'obspy-scan -o ' + self.name + '_scan.pdf ' + self.archive + '/*/' + self.network + '/' + self.name + '/' + ident + '*')
    ...
    def digitizer(self, time='None'):
        if len(self.changetimes) == 0:
            ...
            if time == 'None':
                print(
                    'Error: Digitizer type cannot be uniquely determined (i.e. a device change occurred at some point in time); please provide a timestamp (UTCDateTime object) with the keyword argument time!')
            else:
                # determine which set to use
                indx = time_checker(time, self.changetimes)
    ...
    def datafiles(self, startday, endday, channel='pref', comp='Z'):
        ...
        for ntr in yearlist:
            ...
            if year1 == year2:  # special case...
                llist = glob(self.archive + '/' + str(
                    yr) + '/' + self.network + '/' + self.name + '/' + channel + comp + '.D/*.[0-9][0-9][0-9]')
                for entry in llist:
...
class Catalog:
        ...
        for j in self.events.keys():  # now go through catalog
            ...
            if cumflag:
                if self.events[j].info['cumsum'] == -99 or self.events[j].info['cumsum'] < kwargs['cumsum_min']:
                    ...
                    continue
            if rmsflag:
                if self.events[j].info['RMS_residual'] == -99. or self.events[j].info['RMS_residual'] > kwargs[
                    'rms_max']:
                    del self.events[j]
                    continue
            if gapflag:
            ...
            if timeflag == 1:  # only minimum set
                ...
                if (self.events[j].info['origin_time'] - kwargs['endtime']) > 0. or (
                        self.events[j].info['origin_time'] - kwargs['starttime']) < 0.:
                    del self.events[j]
                    continue
            ...
            if latflag == 1:  # only minimum set
                ...
                if self.events[j].info['event_lat'] > kwargs['lat_max'] or self.events[j].info['event_lat'] < kwargs[
                    'lat_min']:
                    del self.events[j]
                    continue
            ...
            if lonflag == 1:  # only minimum set
                ...
                if self.events[j].info['event_lon'] > kwargs['lon_max'] or self.events[j].info['event_lon'] < kwargs[
                    'lon_min']:
                    del self.events[j]
                    continue
            ...
            if depflag == 1:  # only minimum set
                ...
                if self.events[j].info['event_dep'] > kwargs['dep_max'] or self.events[j].info['event_dep'] < kwargs[
                    'dep_min']:
                    del self.events[j]
                    continue
            ...
            if magflag == 1:  # only minimum set
                ...
                if self.events[j].info['magnitude'] > kwargs['mag_max'] or self.events[j].info['magnitude'] < kwargs[
                    'mag_min']:
                    del self.events[j]
                    continue
    ...
    def filter_slabdist(self, model='slab1.0', dist_min=0., dist_max=999.):
        ...
        for j in self.events.keys():
            ...
            dist = util_c_sippl.compute_point_distance(self.events[j].info['event_lon'],
                                                       self.events[j].info['event_dep'], prof, Lons)
            if dist < dist_min:
                del self.events[j]
                continue
            if dist > dist_max:
        ...
        while key1 < lendic1:
            ...
            while key2 < lendic2:
                ...
                if orig2 > orig + window:
                    ...
                    cat_both1.events[sorted(self.events.keys())[key1]] = cp(
                        self.events[sorted(self.events.keys())[key1]])
                    cat_both2.events[sorted(other.events.keys())[key2]] = cp(
                        other.events[sorted(other.events.keys())[key2]])
                    key1 += 1
                    bkp = cp(key2)
                    flag = True
                    break
    ...
    def plot_map(self, llcrnrlat='none', llcrnrlon='none', urcrnrlat='none', urcrnrlon='none', save=False,
                 colorcode=True, dots=True, mines=False, lock=False, trench=True):
        """
        ...
        m = basemap.Basemap(projection='merc', llcrnrlat=llcrnrlat, llcrnrlon=llcrnrlon, urcrnrlat=urcrnrlat,
                            urcrnrlon=urcrnrlon, resolution='i')
        m.drawcoastlines()
        m.drawcountries()
        ...
        if mines:
            minelats = [-22.310, -22.332, -21.004, -20.9665, -20.9766, -20.886, -22.3844, -20.0504, -20.4077, -23.4303,
                        -22.7908, -21.986, -21.919, -24.269, -20.1952, -22.6819]
            minelons = [-68.908, -69.653, -68.808, -68.721, -69.6824, -70.032, -70.2118, -69.257, -69.868, -70.0568,
                        -69.2552, -68.7008, -68.8307, -69.066, -69.3362, -70.1766]

            nilslats = [-22.2097, -22.3806, -22.5210, -22.8198, -22.6359, -22.5680, -22.6785, -23.4333, -23.4312,
                        -22.9742, -22.7916, -23.1227, -22.8596, -22.1297, -20.8879, -20.9497, -20.9896, -20.9806,
                        -20.8943, -20.4869, -20.4322, -20.2546, -20.2171, -20.0604, -22.6072]
            nilslons = [-69.6510, -70.2116, -69.8789, -69.7205, -69.8829, -69.8125, -70.1691, -70.0635, -69.5130,
                        -69.0622, -69.2572, -69.8556, -69.3253, -69.8607, -70.0320, -69.9924, -69.6557, -69.5920,
                        -69.6541, -69.9489, -69.8669, -69.8339, -69.7951, -69.2697, -69.6721]
    ...
    def plot_ewsection(self, centr_lat='none', width='none', maxlon='none', minlon='none', save=False, mag=True,
                       hist=False, slab=False, slabmod='tassara', ident=False):
        """
        ...
        if not width == 'none':
            mxlat = centr_lat + (width / util_c_sippl.KM_PER_DEG)
            mnlat = centr_lat - (width / util_c_sippl.KM_PER_DEG)
        ...
        fac = cos(lat_mean * pi / 180.) * util_c_sippl.KM_PER_DEG
        ...
        if hist:
            hist2d(((array(lons) - mnlon) * fac), (array(deps) * (-1)), (225, 125), norm=mpl.colors.LogNorm(),
                   cmap=cm.jet)

        elif mag:
            if not ident:
                scatter(((array(lons) - mnlon) * fac), (array(deps) * (-1)), s=array(mags) * 3.5, facecolor='none',
                        edgecolors='black')
            else:
                for j in range(len(lons)):
                    if idents[j] == 'ID':
                        plot((lons[j] - mnlon) * fac, deps[j] * (-1), 'o', markersize=4, markerfacecolor='none',
            ...
            if not ident:
                scatter(((array(lons) - mnlon) * fac), (array(deps) * (-1)), s=0.5, facecolors='none',
                        edgecolors='black')
            else:
                for j in range(len(lons)):
                    if idents[j] == 'ID':
    ...
    def plot_topoprof(centr_lat):
        ...
        prof = util_c_sippl.get_closest_profile(centr_lat, Lats, Lons, Deps)
        fac = cos(centr_lat * pi / 180.) * util_c_sippl.KM_PER_DEG
    ...
    def plot_anysection(self, startpoint, endpoint, width='none', save=False):
        ...
        dx = (xbig - xsmall) * util_c_sippl.KM_PER_DEG * cos((ybig + ysmall) / 2. * pi / 180.)
        dy = (ybig - ysmall) * util_c_sippl.KM_PER_DEG
        try:
        ...
        for j in range(len(lats)):
            ev_offset_n = ((startpoint[0] - lats[j]) * util_c_sippl.KM_PER_DEG) / sin(angle_norm * pi / 180.)
            ev_offset_e = (-1) * ((startpoint[1] - lons[j]) * util_c_sippl.KM_PER_DEG * cos(startpoint[0] * pi / 180.)) / sin(
                (90. - angle_norm) * pi / 180.)

            # check whether event is within profile line
            xstart = 0
            ystart = 0
            xend = (endpoint[1] - startpoint[1]) * util_c_sippl.KM_PER_DEG * cos((endpoint[1] + startpoint[1]) / 2. * pi / 180.)
            yend = (endpoint[0] - startpoint[0]) * util_c_sippl.KM_PER_DEG
            xev = (lons[j] - startpoint[1]) * util_c_sippl.KM_PER_DEG * cos((lons[j] + startpoint[1]) / 2. * pi / 180.)
            yev = (lats[j] - startpoint[0]) * util_c_sippl.KM_PER_DEG

            dist = ((yend - ystart) * xev - (xend - xstart) * yev + xend * ystart - yend * xstart) / sqrt(
                (yend - ystart) ** 2 + (xend - xstart) ** 2)
    ...
    def plot_nssection(self, centr_lon='none', width='none', maxlat='none', minlat='none', save=False, mag=False):
        ...
        fac = util_c_sippl.KM_PER_DEG * cos(mean_lat * pi / 180.)
        ...
        latrange = (mxlat - mnlat) * util_c_sippl.KM_PER_DEG
        ...
        if mag:
            scatter(((array(lats) - mnlat) * util_c_sippl.KM_PER_DEG), (array(deps) * (-1)), s=array(mags) * 4, facecolor='none',
                    edgecolors='black')
        else:
            scatter(((array(lats) - mnlat) * util_c_sippl.KM_PER_DEG), (array(deps) * (-1)), s=0.5, facecolors='none',
                    edgecolors='black')
        ...
        Send = (-24.75 - mnlat) * util_c_sippl.KM_PER_DEG
        Nend = (-18. - mnlat) * util_c_sippl.KM_PER_DEG
    ...
    def plot_histog_mag(self, bins=[], xlimits=[], xticks=[], **kwargs):
        ...
        hy, hx, patches = plt.hist(mag_arr, nbin, range=hrange, normed=hist_normedflag, log=hist_logflag,
                                   cumulative=hist_cumulflag, histtype=hist_typeflag, facecolor='green', alpha=0.75)
        if hist_normedflag:
            ymin = 0
            ymax = hy.max()
            ylabel_str = 'Density'
        ...
        if hist_combinedflag:
            plt.hist(mag_arr, nbin, range=hrange, log=True, cumulative=False, histtype='bar', facecolor='green',
                     alpha=0.75)
            ymin = 1
            ymax = hy.max() + 1000
            ylabel_str = 'Log(Nev)'
    ...
    def plot_histog_cumsum(self, bins=[], xlimits=[], xticks=[], **kwargs):
        ...
        hy, hx, patches = plt.hist(cumsum_arr, nbin, range=hrange, normed=hist_normedflag, log=hist_logflag,
                                   cumulative=hist_cumulflag, histtype=hist_typeflag, facecolor='green', alpha=0.75)
        if hist_normedflag:
            ymin = 0
            ymax = hy.max()
        ...
        if not hist_normedflag and not hist_logflag and not hist_cumulflag and hist_cumulflag != -1:
            ...
            hist_str = 'counts'
        if hist_combinedflag:
            plt.hist(cumsum_arr, nbin, range=hrange, log=True, cumulative=False, histtype='bar', facecolor='green',
                     alpha=0.75)
            ymin = 1
            ymax = hy.max() + 1000
            ylabel_str = 'Log(Nev)'
    ...
    def plot_histog_npicks(self, bins=[], xlimits=[], xticks=[], **kwargs):
        ...
        hy, hx, patches = plt.hist(npick_arr, nbin, range=hrange, normed=hist_normedflag, log=hist_logflag,
                                   cumulative=hist_cumulflag, histtype=hist_typeflag, facecolor='green', alpha=0.75)
        if hist_normedflag:
            ymin = 0
            ymax = hy.max()
        ...
        if not hist_normedflag and not hist_logflag and not hist_cumulflag and hist_cumulflag != -1:
            ...
            hist_str = 'counts'
        if hist_combinedflag:
            plt.hist(npick_arr, nbin, range=hrange, log=True, cumulative=False, histtype='bar', facecolor='green',
                     alpha=0.75)
            ymin = 1
            ymax = hy.max() + 1000
            ylabel_str = 'Log(Nev)'
