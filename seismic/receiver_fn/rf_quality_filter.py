...
@click.option('--filter-band', type=float, nargs=2,
              help="Filter pass band edges (Hz) as a pair of number separated by spaces, e.g. \"0.25 1.0\". "
                   "Leave empty for no filtering. For broadband stations with event magnitude >= 5.5, filtering "
                   "to range \"0.25 1.0\" is indicative for RFs generated by time-domain iterative deconvolution.")
@click.option('--parallel/--no-parallel', default=True, show_default=True, help="Use parallel execution")
def main(input_file, output_file, temp_dir=None, filter_band=None, parallel=True):
