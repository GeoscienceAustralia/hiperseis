...
def test_time_fields(year,month,day,hour,minute,sec,seedyear,strtime,bad_time):
...
def decode_gps(fp,bytes,print_bad_temperture,print_bad_gps,mylat,mylng,myalt,seedyear,strtime,bad_time):

    global good_gps,bad_gps
    ...
    if( temperature < -50)  or (temperature > 150):
        ...
        temperature = 0
    bad_time,strtime = test_time_fields(year,month,day,hour,minute,sec,seedyear,strtime,bad_time)
    if( bad_time == 1 ):
        if print_bad_gps ==1:
...
def decode_message(outcome,size):
    ...
    if outcome == -1:
        ...
        print " Requested ",size," bytes. Only ",outcome," bytes were available "
...
def myhelp():
    print "Program to display contents of the logfile .dat"
    print "Usage"
    print "decode_datfile -f fullpathname_to_log_file [-b yes] [-u yes] [-y year] [-t yes] [-a yes]"
    print "  [] these are optional arguments "
    print "   -u  Print out the gps update info   Default is no"
    print "   -b  Print out bad gps info          Default is no"
    print "   -i  Print out bad id strings        Default is no"
    print "   -t  Print out bad tempetures info   Default is no"
    print "   -a  Print out all                   Default is no"
    print "   -y  Gpsyear - year  -->  max differnce must be 1"
...
SEISTYPES  = ["LennartzLE-3DlITE","GurupCMG-40T","GuralpCMG-3ESP","TrilliumCompact","MarkL4","MarkL4C"]
block = ['.','.','.']
print_bad_gps = 0
print_gps_update = 0
gps_update_failed = 0
...
#    filename = tkFileDialog.askopenfilename(parent=root,initialdir='/space3/',title=gpstext )
#    filename = tkFileDialog.askopenfilename(parent=root,initialdir=startdir,title=gpstext )
try:
    fp = open(filename,"rb")
except IOError:
...
headertest,recstr = test_fileformat_start(fp,filename)
if headertest < 0:
    fp.close()
    sys.exit(100)
elif headertest == 1:
    print "********* WARNNING WARNINIG"
    print "********* WARNNING WARNNING Indication first 8 chars match a minseed file -->",recstr
...
while 1:
    ...
    if   id_str == 'BSN':
        ...
        if not (flagmarker & 0x0001):
            print "####################################################################################"
            print "   Seedyear was ",seedyear,"                    To change use -y year option"
        ...
        first_gps = 0
        (strtime,mylat,mylng,myalt) = decode_gps(fp,60,print_bad_temperture,print_bad_gps,mylat,mylng,myalt,seedyear,strtime,bad_time)
        if(len(strtime) > 0):
            if not strtime[1] == 0 and not strtime[2] == 0 and not strtime[2] == 0:
...
print "     ", good_gps+bad_gps,"     ",good_gps,"      ",bad_gps,"       ",gps_update_failed
...
#    print "Geo Coords:   ",'%.5f'%(sum(mylat)/len(mylat)),"  ",'%.5f'%(sum(mylng)/len(mylng)),"   ",int(sum(myalt)/len(myalt))
    ...
    print "      MEDIAN VALS:",'   %.5f'%median_lat,"              ",'%.5f'% median_lng,"               ",'%.2f' % median_alt
print "####################################################################################"
if print_badid_update == 1:
...
if restarts > 0:
    ...
    if restarts > 1:
        print "WARNING File ",filename," restarted!. Marks at location "
