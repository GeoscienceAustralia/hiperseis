...
def plot_rf_stack(rf_stream, time_window=(-10.0, 30.0), trace_height=0.2, stack_height=0.8, save_file=None, **kwargs):
    ...
    stackable_stream = rf_util.get_stackable_stream(rf_stream)
...
#=================================================================
# plotting routines for 2-stage stacking (Zhang and Olugboji 2021)
#=================================================================
def plot_k_vp_stack(ax, k_grid, vp_grid, stack, title=None, num=None,
                    clip_negative=True):
    """Plot H-k stack using data generated by function seismic.receiver_fn.rf_stacking.computed_weighted_stack().

    :param k_grid: Grid of k-values
    :type k_grid: Two-dimensional numpy.array
    :param vp_grid: Grid of Vp-values
    :type vp_grid: Two-dimensional numpy.array
    :param stack: Grid of stacked RF sample values produced by function
        seismic.receiver_fn.rf_stacking.estimate_k_vp()
    :type stack: Two-dimensional numpy.array
    :param title: Title to add to the plot, defaults to None
    :type title: str, optional
    :param num: Number of RFs used to produce the stack, defaults to None
    :type num: int, optional
    ...
    cs = ax.contourf(k_grid, vp_grid, stack, levels=20, cmap=cmap)
    for c in cs.collections:
        c.set_rasterized(True)
    # end for

    cb = ax.get_figure().colorbar(cs, ax=ax, orientation='horizontal')

    cb.ax.set_xlabel('Stack sum')
    ax.set_xlabel(r'$\kappa = \frac{V_p}{V_s}$ (ratio)', fontsize=14)
    ax.set_ylabel('$V_p$ (km/s)', fontsize=14)
    if title is not None:
        ax.set_title(title, fontsize=16)
    ax.tick_params(axis='both', which='major', labelsize=14)
    ax.set_xlim(np.min(k_grid), np.max(k_grid))
    ax.set_ylim(np.min(vp_grid), np.max(vp_grid))

    ax.scatter(k_grid[mi], vp_grid[mi], s=20, marker='+', c='k')

    xl = ax.get_xlim()
    x = np.mean(np.array(ax.get_xlim()))
    x1 = x + (xl[1] - x) * 0.55
    ax.text(x, Vp, r"$\kappa$={:.3f}, $V_p$={:.3f}".format(k, Vp),
    ...
    if num is not None:
        ...
        txt_x = xl[0] + 0.95 * (xl[1] - xl[0])
        txt_y = yl[0] + 0.95 * (yl[1] - yl[0])
        ax.text(txt_x, txt_y, "N = {}".format(num), horizontalalignment='right',
                color="#ffffff", fontsize=16, fontweight='bold', rasterized=True)
    # end if
# end func

def plot_h_estimates(ax, h_grid, stack, hlist, title=None):
    ax.plot(stack, h_grid)
    if title is not None:
        ax.set_title(title, fontsize=16)
    ax.tick_params(axis='both', which='major', labelsize=14)
    ax.set_xlabel('Stack sum', fontsize=14)
    ax.set_ylabel(r'H = Moho depth (km)', fontsize=14)

    if (len(hlist)):
        x = np.mean(np.array(ax.get_xlim()))
        for ih, h in enumerate(hlist):
            ax.text(x, h + 0.5, r"H{}={:.3f}".format(ih, h),
                    color="#000000", fontsize=12, horizontalalignment='left',
                    clip_on=False)

            xmax = stack[np.argmin(np.fabs(h_grid - h))]
            ax.hlines(y=h, xmin=np.min(stack), xmax=xmax, linestyle='--', color='grey')
        # end for
    # end if
# end func
